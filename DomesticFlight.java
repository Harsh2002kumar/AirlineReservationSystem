...
public class DomesticFlight extends JFrame
{
	...
	public DomesticFlight(LoginPage type1)
	{
		...
		String regexPattern = "^(?:(?:31(\\/|-|\\.)(?:0?[13578]|1[02]))\\1|(?:(?:29|30)(\\/|-|\\.)(?:0?[13-9]|1[0-2])\\2))(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$|^(?:29(\\/|-|\\.)0?2\\3(?:(?:(?:1[6-9]|[2-9]\\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\\d|2[0-8])(\\/|-|\\.)(?:(?:0?[1-9])|(?:1[0-2]))\\4(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$";
...
//		BFindFlight.addActionListener(new button3(this, type1));
		BFindFlight.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String sDate = TFBookingDate.getText();

				// Check for valid date format (DD/MM/YYYY)
//				String regexPattern = "^(((0|1)[0-9]|2[0-9]|3[0-1])\/(0[1-9]|1[0-2])\/((19|20)\d\d))$";
				Pattern pattern = Pattern.compile(regexPattern);
				Matcher matcher = pattern.matcher(sDate);

				if (!matcher.matches()) {
					// Display error message for invalid date format
					JOptionPane.showMessageDialog(DomesticFlight.this, "Invalid Date format. Please enter in DD/MM/YYYY format.", "Error", JOptionPane.ERROR_MESSAGE);
					return;  // Exit the action listener if date is invalid
				}else{
					new button3(DomesticFlight.this, type1).actionPerformed(e);  // Pass the event object

				}

				// ... rest of the button click functionality (assuming valid date) ...
			}
		});
	}
